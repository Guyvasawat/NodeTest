...
exports.resetpassword = async (req, res, next) => {
    ...
    try {
        ...
        if (row.length == 1 ) {
            ...
            const [keep] = await dbConnection.execute('SELECT `password` FROM `users` WHERE `email` =?', [body._email]);
            console.log(keep[0].password);
            if(body._passreset == body._passconfirm){
                const hashPass_reset = await bcrypt.hash(body._passreset, 12);
                console.log(hashPass_reset);
                const [newpass] = await dbConnection.execute(
                    "UPDATE `users`SET `password` =? WHERE `email` =?",
                    [hashPass_reset, body._email]
                );
